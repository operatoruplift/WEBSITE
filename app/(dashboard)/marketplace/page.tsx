        ...
        <MobilePageWrapper>
            <div className="min-h-screen p-6 lg:p-8 relative">
                ...
                <div className="max-w-7xl mx-auto space-y-8 relative z-10">
                    ...
                    <div className="flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center bg-foreground/[0.04] p-2 rounded-2xl border border-foreground/10 backdrop-blur-md animate-fadeInUp" style={{ animationDelay: '200ms' }}>
                    ...
                    {category === 'All' && !search && <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fadeInUp" style={{ animationDelay: '300ms' }}>
                        {isLoading ? Array(2).fill(0).map((_, i) => <div key={i} className="h-64 rounded-2xl bg-foreground/[0.04] border border-foreground/10 animate-pulse" />) :
                            featuredAgents.slice(0, 2).map(agent => (
                                <Card key={agent.id} variant="glass" className="relative overflow-hidden group border-white/10 hover:border-[#F97316]/30 transition-all duration-700 cursor-pointer">
                                    ...
                                    <CardContent className="p-8 relative z-10 flex flex-col h-full justify-between">
                                        <div><div className="flex items-start gap-5"><div className="w-16 h-16 rounded-2xl bg-foreground/[0.04] border border-white/10 flex items-center justify-center text-3xl shadow-xl group-hover:scale-110 transition-all duration-500">{agent.avatar}</div><div className="flex-1 mt-1"><div className="flex items-center gap-2 mb-1"><h3 className="text-xl font-medium text-white tracking-tight">{agent.name}</h3>{agent.verified && <Check size={14} className="text-[#F59E0B]" />}</div><p className="text-xs font-mono text-gray-500 uppercase tracking-wider">{agent.author}</p></div></div><p className="text-sm text-gray-300 mt-5 leading-relaxed">{agent.description}</p></div>
                    ...
                    <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 animate-fadeInUp relative z-10' : 'space-y-3 animate-fadeInUp relative z-10'} style={{ animationDelay: '400ms' }}>
                        {isLoading ? Array(8).fill(0).map((_, i) => <div key={i} className="h-48 rounded-2xl bg-foreground/[0.04] border border-foreground/10 animate-pulse" />) :
