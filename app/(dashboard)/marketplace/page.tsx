...
const priceColors: Record<string, string> = { free: 'text-green-400 bg-green-400/10 border border-green-400/20', pro: 'text-[#F08A4C] bg-[#F08A4C]/10 border border-[#F08A4C]/20', enterprise: 'text-[#F08A4C] bg-[#F08A4C]/10 border border-[#F08A4C]/20' };
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div>
                            <div className="flex items-center gap-3 mb-2">
                                <div className="w-10 h-10 rounded-xl flex items-center justify-center relative" style={{ background: 'linear-gradient(135deg, #F08A4C, #F08A4C)' }}><Store size={20} className="text-white" /></div>
                                <Badge variant="default" className="bg-[#F08A4C]/10 text-[#F08A4C] border-[#F08A4C]/20 text-[10px] tracking-widest font-mono">v2.0</Badge>
                        ...
                        <Link href="/agents/builder"><GlowButton className="h-11 px-6 bg-foreground/[0.04] border border-white/10 hover:bg-white/10 group transition-all"><Sparkles size={16} className="mr-2 text-[#F08A4C]" /><span className="font-medium tracking-wide">Build Your Own</span></GlowButton></Link>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4" style={{ animationDelay: '100ms' }}>
                        {[{ label: 'Available Agents', value: '2,400+', icon: Bot, gradient: 'from-[#F08A4C]/20 to-transparent', color: 'text-[#F08A4C]' },
                          { label: 'Total Installs', value: '1.2M', icon: Download, gradient: 'from-emerald-500/20 to-transparent', color: 'text-emerald-400' },
                          { label: 'Active Developers', value: '8,200', icon: Users, gradient: 'from-[#F08A4C]/20 to-transparent', color: 'text-[#F08A4C]' },
                          { label: 'Avg. Rating', value: '4.7', icon: Star, gradient: 'from-[#F08A4C]/20 to-transparent', color: 'text-[#F08A4C]' }
                    ...
                    {category === 'All' && !search && <div className="grid grid-cols-1 lg:grid-cols-2 gap-6" style={{ animationDelay: '300ms' }}>
                        {isLoading ? Array(2).fill(0).map((_, i) => <div key={i} className="h-64 rounded-2xl bg-foreground/[0.04] border border-foreground/10" />) :
                            featuredAgents.slice(0, 2).map(agent => (
                                <Card key={agent.id} variant="glass" className="relative overflow-hidden group border-white/10 hover:border-[#F08A4C]/30 transition-all duration-700 cursor-pointer">
                                    <BorderBeam size={250} duration={8} colorFrom="#F08A4C" colorTo="#F08A4C" />
                                    <div className="absolute inset-0 bg-gradient-to-br from-[#F08A4C]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                                    <div className="absolute top-0 right-0 px-4 py-1.5 bg-gradient-to-r from-[#F08A4C] to-[#F08A4C] text-white text-[10px] uppercase font-bold tracking-widest rounded-bl-xl shadow-lg flex items-center gap-1.5 z-20"><Crown size={12} /> TOP TIER</div>
                                    <CardContent className="p-8 relative z-10 flex flex-col h-full justify-between">
                                        <div><div className="flex items-start gap-5"><div className="w-16 h-16 rounded-2xl bg-foreground/[0.04] border border-white/10 flex items-center justify-center text-3xl shadow-xl group- transition-all duration-500">{agent.avatar}</div><div className="flex-1 mt-1"><div className="flex items-center gap-2 mb-1"><h3 className="text-xl font-medium text-white tracking-tight">{agent.name}</h3>{agent.verified && <Check size={14} className="text-[#F08A4C]" />}</div><p className="text-xs font-mono text-gray-500 uppercase tracking-wider">{agent.author}</p></div></div><p className="text-sm text-gray-300 mt-5 leading-relaxed">{agent.description}</p></div>
