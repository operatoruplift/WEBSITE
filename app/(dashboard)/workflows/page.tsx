                ...
                <div className="max-w-7xl mx-auto space-y-8">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                <GitBranch size={16} className="text-[#F08A4C]" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {[
                            ...
                            { label: 'Total Runs', value: workflows.reduce((sum, w) => sum + w.runs, 0).toLocaleString(), icon: Zap, color: 'text-[#F08A4C]' },
                            { label: 'Total Steps', value: workflows.reduce((sum, w) => sum + w.steps, 0), icon: GitBranch, color: 'text-[#F08A4C]' },
                        ].map(stat => {
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                            ...
                                            <div className="flex items-center gap-2 flex-shrink-0">
                                                <button onClick={() => runWorkflow(wf.id)} disabled={runningId === wf.id || !!runningId}
                                                    className={`px-3 py-2 rounded-lg text-xs font-bold transition-colors ${runningId === wf.id ? 'bg-[#F08A4C]/20 text-[#F08A4C] cursor-wait' : 'bg-[#F08A4C]/10 text-[#F08A4C] hover:bg-[#F08A4C]/20'}`}>
                                            ...
                                            {(runningId === wf.id || (stepOutputs[wf.id] && stepOutputs[wf.id].length > 0)) && (
                                                <div className="w-full mt-3 space-y-2">
                                                    {runningId === wf.id && (
                                                        <div className="w-full h-1 bg-foreground/[0.04] rounded-full overflow-hidden"><div className="h-full bg-[#F08A4C] rounded-full transition-all duration-500" style={{ width: `${runProgress}%` }} /></div>
                                                    )}
