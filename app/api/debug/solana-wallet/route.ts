 ...
 */
function requireAdminKey(request: Request): NextResponse | null {
    const expected = process.env.ADMIN_DEBUG_KEY;
    if (!expected) {
        return NextResponse.json({ error: 'admin_debug_not_configured', message: 'Set ADMIN_DEBUG_KEY in Vercel env to enable /api/debug/*.' }, { status: 503 });
    }
...
export async function GET(request: Request) {
    const block = requireAdminKey(request);
