    ...
    return (
        <div className="w-full bg-background min-h-screen">
            ...
            <div className="pt-24 pb-0 px-4 md:px-8 max-w-[1200px] mx-auto flex flex-col lg:flex-row gap-8">
                <aside className="w-full lg:w-64 shrink-0 lg:sticky lg:top-24 lg:self-start lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto border-b lg:border-b-0 lg:border-r border-white/10 pb-6 lg:pb-10 lg:pr-6">
                    <div className="text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-4">Docs</div>
                    <nav className="space-y-6">
                        {DOC_GROUPS.map(group => {
                            ...
                            return (
                                <div key={group}>
                                    <div className="text-[10px] font-mono uppercase tracking-wider text-gray-500 mb-2 px-2">{group}</div>
                                    <ul className="space-y-0.5">
                                        {entries.map(s => (
                                            <li key={s.slug}>
                                                <Link
                                                    href={`/docs/${s.slug}`}
                                                    className="block px-2 py-1.5 rounded text-sm text-gray-300 hover:text-white hover:bg-white/[0.04] transition-colors"
                                                >
                                                    {s.title}
                                                </Link>
                                            </li>
