...
| **Agent ID** | **Ship (testnet)** | Additive: our existing ERC-8004-style agent registration documents (`agents/*.json`) get a parallel 0G Agent ID registration so the same agent identity is verifiable from a second standard. Implemented in `lib/og/agent-id.ts` + `scripts/og-agent-id-mint.mjs` + `data/og-agent-ids.json` + `/agents/{slug}.json` field. Operator action remaining: fund `OG_PRIVATE_KEY` wallet at `https://faucet.0g.ai` and run the mint script once. |
