...
| `OG_PRIVATE_KEY` (+ optional `OG_RPC_URL`, `OG_INDEXER_RPC`) | **REQUIRED for 0G Storage anchoring (0G Labs hackathon integration)** | `lib/og/storage.ts`, `/api/cron/og-anchor` | Wallet private key for 0G testnet. Without this, receipts ship signed but never anchor to 0G; the `0g: <rootHash>` link on `/security` stays hidden (same hide-when-NULL contract as Filecoin). Optional overrides default to `https://evmrpc-testnet.0g.ai` and `https://indexer-storage-testnet-turbo.0g.ai`. After setting, run `lib/og-storage-migration.sql` in Supabase and curl `/api/cron/og-anchor` once. See `docs/0g-integration-decision.md` for the full scope. |
| **0G Agent ID mint (one-shot, operator-side)** | OPTIONAL — adds an on-chain identity for each agent | `scripts/og-agent-id-mint.mjs`, `data/og-agent-ids.json`, `/agents/{slug}.json` | Mints an ERC-7857 Intelligent NFT for each agent into the 0G Foundation reference AgenticID contract (`0x2700F6A3e505402C9daB154C5c6ab9cAEC98EF1F` on Galileo Testnet). Re-uses `OG_PRIVATE_KEY` — fund the wallet from `https://faucet.0g.ai`, then run `node scripts/og-agent-id-mint.mjs`. The script writes resulting tokenIds back to `data/og-agent-ids.json`, which is committed so the public agent JSON manifests include the on-chain pointer + chainscan link. Until run, the `og_agent_id` field is simply omitted from the JSON (honest by default). Optional overrides: `OG_AGENT_ID_CONTRACT` to use your own deployment instead of the reference. |
| `ARKIV_PRIVATE_KEY` (+ optional `NEXT_PUBLIC_ARKIV_CREATOR_ADDRESS`) | **REQUIRED for Arkiv writes (Network School Ethereum Hackathon (AI track) entrant)** | `lib/arkiv/client.ts`, `scripts/arkiv/publish-agents.mjs`, `/api/arkiv/agents`, `/api/arkiv/memories` | Wallet private key for the Arkiv Braga testnet (chain id `60138453102`). Without this, the read routes still respond 200 with empty envelopes (`{ count: 0 }`); the `/arkiv` page shows the honest empty state instead of fabricating entities. Fund the wallet at `https://braga.hoodi.arkiv.network/faucet/`, then run `pnpm arkiv:publish-agents` to mirror `/agents/calendar.json` and `/agents/gmail.json` onto Braga as on-chain identity cards. PROJECT_ATTRIBUTE `operatoruplift-bucharest-arkiv-7q3w` is enforced on every write + query; do not change it without also updating `lib/arkiv/constants.ts`. The public creator address can be surfaced via the optional `NEXT_PUBLIC_ARKIV_CREATOR_ADDRESS` env var (display only). See `docs/ARKIV_SUBMISSION.md` for the full scope. |
| **Arkiv publish (one-shot, operator-side)** | OPTIONAL — populates the `/arkiv` entity list | `scripts/arkiv/publish-agents.mjs` | Runs after `ARKIV_PRIVATE_KEY` is funded. Reads `/agents/{slug}.json` (Calendar + Gmail), computes the SHA-256 checksum, writes one `agent` entity per slug to Braga under the project key. Re-running publishes a new entity per slug (versioned via Arkiv's entity history). Verify with `pnpm arkiv:smoke` before and after. |
