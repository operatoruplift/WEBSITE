...
| 0G Agent ID on-chain identity (ERC-7857) | Scaffolded — operator action to activate | `scripts/og-agent-id-mint.mjs` mints an Intelligent NFT for each agent into the 0G Foundation reference AgenticID contract `0x2700F6A3e505402C9daB154C5c6ab9cAEC98EF1F` on Galileo Testnet. TokenIds persist in `data/og-agent-ids.json` (committed) and surface on `/agents/{slug}.json` as the `og_agent_id` field with a chainscan link. Until the operator funds `OG_PRIVATE_KEY` at `https://faucet.0g.ai` and runs the mint script, the field is omitted from the JSON — the deploy never claims a tokenId it does not have. |
...
| Arkiv on-chain agent identity cards + memory events | Real | `lib/arkiv/` (constants, client, agent, memory) + script at `scripts/arkiv/publish-agents.mjs` + public read routes at `/api/arkiv/agents` and `/api/arkiv/memories` + judge demo at `/arkiv`. Braga testnet. Active when `ARKIV_PRIVATE_KEY` is funded. Honest empty state when unset: `/api/arkiv/agents` returns `{ agents: [], count: 0 }` and `/arkiv` shows "no entities yet" instead of fabricating data. PROJECT_ATTRIBUTE `operatoruplift-bucharest-arkiv-7q3w` enforced on every write and every read. |
