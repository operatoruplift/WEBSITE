...
| 12a | Arkiv Network School Ethereum Hackathon entrant shipped (AI theme) | PRs #620..#637 wired `lib/arkiv/` + `/api/arkiv/agents` + `/api/arkiv/memories` + `/arkiv` demo + `scripts/arkiv/publish-agents.mjs`. `docs/ARKIV_SUBMISSION.md` documents the full submission. Operator action: set `ARKIV_PRIVATE_KEY` in Vercel, fund the wallet at `https://braga.hoodi.arkiv.network/faucet/`, run `pnpm arkiv:publish-agents`. Verify with `pnpm arkiv:smoke`. Until then, `/arkiv` and the API routes return honest empty envelopes. | **operator** |
