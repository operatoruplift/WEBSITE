...
function getSystem(user?: ImessageUser | null): string {
    // Per-user override wins over env override wins over default.
    // Stack a small "what we know about you" block when the user has
    // location / timezone / zodiac / summary set, so the agent can
    // weave those in without the operator having to write a custom
    // system prompt for every user.
