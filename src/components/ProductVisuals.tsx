...
export const RuntimeVisual = () => {
  ...
  // 14-tick cycle (14s): ticks 0-3 = steps running (4s), ticks 4-13 = complete hold (10s)
...
export const TokenVisual = () => {
  ...
  // 16-tick cycle (16s): ticks 0-6 = keys visible (7s), ticks 7-15 = locked (9s)
  const cyclePos = tick % 16;
  const showKeys = cyclePos < 7;
...
export const PermissionsVisual = () => {
  ...
  useEffect(() => {
    ...
    const runCycle = () => {
      setStatus('PENDING');
      t1 = setTimeout(() => setStatus('REQUESTING'), 2500);
      t2 = setTimeout(() => setStatus('ALLOWED'), 5000);
      t3 = setTimeout(() => runCycle(), 16000);
    };
