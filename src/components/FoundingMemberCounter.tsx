...
/**
 * FoundingMemberCounter , live waitlist count rendered as a hero
 * trust signal. Replaces the static "53/100 Private Beta Spots
 ...
 *   - Counts up from 0 to the fetched total over ~900ms on mount.
 ...
 *
 * Data: GET /api/waitlist/counts returns `{ total, founder, ... }`.
 *   - `total` is the rendered count (every signup counts as a
 *     founding member at this phase).
 *   - The component hides itself when the endpoint returns 0 or
 *     errors , same honest-empty pattern as HeroCountsPill (#770)
 *     and /waitlist's counts pill (app/waitlist/page.tsx:124).
 */
const FoundingMemberCounter: React.FC = () => {
    ...
    useEffect(() => {
        ...
        fetch('/api/waitlist/counts', { cache: 'no-store' })
            ...
            .catch(() => {
                // Silently keep target null; the component renders nothing.
            });
        return () => {
            cancelled = true;
        };
